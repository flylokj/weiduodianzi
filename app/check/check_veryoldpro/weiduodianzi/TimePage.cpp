...
void TimePage::initFocusList()
{
	...
	QString temp = DataBase::getInstance()->queryData("instYear");
	temp += "-";
	...
	//系统累计使用时间显示;
	int usedTime = DataBase::getInstance()->queryData("usedTime").toInt();
	usedTime /= TIME_UNIT_BASE;
	ui.usedTimeLbl->setText(QString::number(usedTime)+QString("h"));

	usedTime = DataBase::getInstance()->queryData("xeUsedTime").toInt();
	usedTime /= TIME_UNIT_BASE;
	ui.lpUsedLbl0->setText(QString::number(usedTime)+QString("h"));

	usedTime = DataBase::getInstance()->queryData("tunUsedTime").toInt();
	usedTime /= TIME_UNIT_BASE;
	ui.lpUsedLbl->setText(QString::number(usedTime)+QString("h"));
...
void TimePage::on_historyBtn_clicked()
{
	//qDebug()<<"history";
